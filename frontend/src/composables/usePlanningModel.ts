...
import {
  ...
  formatTimeRange as formatTimeRangeValue,
  getFestivalDisplayInfo,
...
export function usePlanningModel() {
  ...
  function classifyTimePreference(
    startMinutes: number,
    avoidBeforeMinutes: number | null,
    avoidAfterMinutes: number | null,
  ): "early" | "late" | null {
    const festivalAvoidBeforeMinutes =
      toFestivalPreferenceMinutes(avoidBeforeMinutes);
    const festivalAvoidAfterMinutes =
      toFestivalPreferenceMinutes(avoidAfterMinutes);

    if (
      festivalAvoidBeforeMinutes === null &&
      festivalAvoidAfterMinutes === null
    ) {
      return null;
    }
    ...
    if (
      startMinutes < festivalAvoidBeforeMinutes! &&
      startMinutes >= festivalAvoidAfterMinutes!
    ) {
      const distanceToLateBoundary = startMinutes - festivalAvoidAfterMinutes!;
      const distanceToEarlyBoundary =
        festivalAvoidBeforeMinutes! - startMinutes;
      return distanceToLateBoundary <= distanceToEarlyBoundary
        ? "late"
        : "early";
    }
  ...
  function screeningRecommendationScore(
    screening: Screening,
    ...
    const hasConflict = selectedScreenings.some(
      (other) =>
    ...
    const comfortBonus =
      settings.preferredVenueScores[screening.venue_name ?? ""] ?? 0;
    const startMinutes = toMinutes(screening.starts_at);
    const timePreference = classifyTimePreference(
      startMinutes,
      settings.avoidBeforeMinutes,
    ...
    if (comfortBonus > 0) {
      reasons.push(
        `salle ${screening.venue_name || "inconnue"} marquée comme favorable`,
      );
    } else if (comfortBonus < 0) {
      drawbacks.push(
        `salle ${screening.venue_name || "inconnue"} marquée comme inconfortable`,
      );
    }
    ...
    return {
      ...
      note:
        total > 0
          ? "option favorable selon tes préférences"
          : "option neutre selon tes préférences",
      reasons,
      drawbacks,
      hasConflict,
    };
  ...
  function countConflictPairs(screenings: Screening[]): number {
    ...
    for (let index = 0; index < screenings.length; index += 1) {
      for (
        ...
        otherIndex < screenings.length;
        otherIndex += 1
      ) {
        if (
          screeningsOverlapWithBuffer(screenings[index], screenings[otherIndex])
        ) {
          count += 1;
        }
      }
  ...
  const effectivePlanningMode = computed(() =>
    isMobile.value && planningMode.value === "visualization"
      ? "timeline"
      : planningMode.value,
  );

  const filmById = computed(
    () => new Map(store.films.map((film) => [film.id, film])),
  );
    ...
    () =>
      new Set(
        store.films
          .filter(
            (film) =>
              isPlannableFilm(film) && isPlanningPriority(film.priority),
          )
          .map((film) => film.id),
      ),
  );
  ...
  const planningScreenings = computed<PlanningScreening[]>(() => {
    const baseScreenings = store.visibleScreenings.filter(
      (screening) =>
        screening.starts_at &&
        screening.ends_at &&
        plannableFilmIds.value.has(screening.film_id),
    );
    const selectedScreenings = baseScreenings.filter(
      (screening) =>
        screening.selection_status === "tentative" ||
        screening.selection_status === "confirmed",
    );
    const selectedFilmIds = new Set(
      selectedScreenings.map((screening) => screening.film_id),
    );
    ...
    for (const screening of baseScreenings) {
      totalScreeningCountByFilmId.set(
        screening.film_id,
        (totalScreeningCountByFilmId.get(screening.film_id) ?? 0) + 1,
      );

      if (screening.selection_status !== "rejected") {
        validScreeningCountByFilmId.set(
          screening.film_id,
          (validScreeningCountByFilmId.get(screening.film_id) ?? 0) + 1,
        );
    ...
    for (const screening of baseScreenings) {
      ...
      candidatesByFilmId.set(screening.film_id, [
        ...(candidatesByFilmId.get(screening.film_id) ?? []),
        screening,
      ]);
    }

    const scoredCandidatesByFilmId = new Map<
      number,
      Array<
        ReturnType<typeof screeningRecommendationScore> & {
          screening: Screening;
        }
      >
    >();
    ...
    for (const [filmId, screenings] of candidatesByFilmId.entries()) {
      if (settingsStore.recommendationMode === "personalized") {
        const hasSelected = screenings.some(
          (screening) =>
            screening.selection_status === "tentative" ||
            screening.selection_status === "confirmed",
        );
        const selectedOtherFilmScreenings = selectedScreenings.filter(
          (screening) => screening.film_id !== filmId,
        );
        const scored = [...screenings]
          .map((screening) => ({
            screening,
            ...screeningRecommendationScore(
              screening,
              selectedOtherFilmScreenings,
            ),
          }))
          .sort(
            (left, right) =>
              right.score - left.score ||
              (left.screening.starts_at ?? "").localeCompare(
                right.screening.starts_at ?? "",
              ),
          );
        ...
        scored.forEach((entry, index) => {
          ...
          recommendationTotalByScreeningId.set(
            entry.screening.id,
            scored.length,
          );
          recommendationNoteByScreeningId.set(entry.screening.id, entry.note);
          recommendationReasonsByScreeningId.set(
            entry.screening.id,
            entry.reasons,
          );
          recommendationDrawbacksByScreeningId.set(
            entry.screening.id,
            entry.drawbacks,
          );
        });
    ...
    const filmIdsToRecommend = [...scoredCandidatesByFilmId.entries()]
      .filter(([filmId, scored]) => {
        const hasSelected = scored.some(
          (entry) =>
            entry.screening.selection_status === "tentative" ||
            entry.screening.selection_status === "confirmed",
        );
    ...
    for (const [filmId, scored] of filmIdsToRecommend) {
      const recommendable = scored.find(
        (entry) =>
          !entry.hasConflict &&
          !recommendedScreenings.some((other) =>
            screeningsOverlapWithBuffer(entry.screening, other),
          ),
      );
      ...
      for (const entry of scored) {
        if (
          entry.screening.id === recommendable.screening.id ||
          entry.hasConflict
        )
          continue;
        const blockingRecommendation = recommendedScreenings.find(
          (other) =>
            other.id !== entry.screening.id &&
            screeningsOverlapWithBuffer(entry.screening, other),
        );
    ...
    return baseScreenings
      .map((screening) => {
        ...
        const totalScreeningCount =
          totalScreeningCountByFilmId.get(screening.film_id) ?? 0;
        const validScreeningCount =
          validScreeningCountByFilmId.get(screening.film_id) ?? 0;
        const lastViableScreeningId =
          lastViableScreeningIdByFilmId.get(screening.film_id) ?? null;
        const isSingleScreening = totalScreeningCount === 1;
        const isLastViableOption =
          totalScreeningCount > 1 && validScreeningCount === 1;
        const isMustLock =
          isLastViableOption &&
          lastViableScreeningId === screening.id &&
          screening.selection_status !== "rejected" &&
          isHighPriority(film?.priority) &&
          !isSelected;
        const isRecommended =
          recommendationByFilmId.get(screening.film_id) === screening.id &&
          !isSelected;
        ...
        return {
          ...
          isSelected,
          isConflict: selectedScreenings.some((other) =>
            screeningsOverlapWithBuffer(screening, other),
          ),
          isAlternative:
            selectedFilmIds.has(screening.film_id) &&
          ...
          isRecommended,
          recommendationRank:
            recommendationRankByScreeningId.get(screening.id) ?? null,
          recommendationTotalOptions:
            recommendationTotalByScreeningId.get(screening.id) ?? null,
          recommendationNote:
            recommendationNoteByScreeningId.get(screening.id) ?? null,
          recommendationReasons:
            recommendationReasonsByScreeningId.get(screening.id) ?? [],
          recommendationDrawbacks:
            recommendationDrawbacksByScreeningId.get(screening.id) ?? [],
          recommendationBlockedBy:
            recommendationBlockedByScreeningId.get(screening.id) ?? null,
        };
      })
      .sort(
        (left, right) =>
          ...
          left.film_title.localeCompare(right.film_title),
      );
  });

  const dayKeys = computed(() => [
    ...new Set(planningScreenings.value.map((screening) => screening.dayKey)),
  ]);
  ...
  const filteredScreenings = computed(() => {
    if (screeningFilter.value === "confirmed") {
      return planningScreenings.value.filter(
        (screening) => screening.selection_status === "confirmed",
      );
    }
  ...
  const dayScreenings = computed(() =>
    activeDay.value === FESTIVAL_VIEW_KEY
      ? filteredScreenings.value
      : filteredScreenings.value.filter(
          (screening) => screening.dayKey === activeDay.value,
        ),
  );
  ...
  const timelineGroups = computed(() => {
    ...
    for (const screening of dayScreenings.value) {
      grouped.set(screening.dayKey, [
        ...(grouped.get(screening.dayKey) ?? []),
        screening,
      ]);
    }
  ...
  const detailScreening = computed(
    () =>
      planningScreenings.value.find(
        (screening) => screening.id === detailScreeningId.value,
      ) ?? null,
  );
  ...
  const relatedFilmScreenings = computed(() => {
    ...
    return filteredScreenings.value
      .filter(
        (screening) => screening.film_id === detailScreening.value?.film_id,
      )
      .sort(
        (left, right) =>
          left.dayKey.localeCompare(right.dayKey) ||
  ...
  const conflictingSelectedScreenings = computed(() =>
    planningScreenings.value.filter(
      (screening) => screening.isSelected && screening.isConflict,
    ),
  );

  const arbitrableScreenings = computed(() =>
    planningScreenings.value.filter(
      (screening) =>
  ...
  const selectedConflictCount = computed(() => {
    const selected = planningScreenings.value.filter(
      (screening) =>
        screening.selection_status === "tentative" ||
        screening.selection_status === "confirmed",
    );
  ...
  const summary = computed(() => ({
    films: store.films.filter(
      (film) => isPlannableFilm(film) && isPlanningPriority(film.priority),
    ).length,
    screenings: planningScreenings.value.length,
    selected: planningScreenings.value.filter(
      (screening) => screening.isSelected,
    ).length,
    conflicts: selectedConflictCount.value,
    toPlace: planningScreenings.value.filter(
      (screening) =>
        !screening.isSelected &&
        !screening.isAlternative &&
        screening.selection_status !== "rejected",
  ...
  const daySummary = computed(() => ({
    ...
    selected: dayScreenings.value.filter((screening) => screening.isSelected)
      .length,
    conflicts: countConflictPairs(
      dayScreenings.value.filter(
        (screening) =>
          screening.selection_status === "tentative" ||
          screening.selection_status === "confirmed",
      ),
  ...
  const visualizationGroups = computed(() => {
    const sourceGroups =
      activeDay.value === FESTIVAL_VIEW_KEY
        ...
        : [{ dayKey: activeDay.value, screenings: dayScreenings.value }];
    ...
    return sourceGroups.map((group) => {
      ...
      if (!allScreenings.length) {
        return {
          ...
          lanes: [] as Array<{
            venueName: string;
            blocks: PlanningScreening[];
          }>,
        };
      }

      const minMinutes = Math.min(
        ...allScreenings.map((screening) => screening.startMinutes),
      );
      const maxMinutes = Math.max(
        ...allScreenings.map((screening) => screening.endMinutes),
      );
      ...
      const venueNames = [
        ...new Set(
          allScreenings.map(
            (screening) => screening.venue_name || "Salle inconnue",
          ),
        ),
      ...
      return {
        ...
        lanes: venueNames.map((venueName) => ({
          ...
          blocks: allScreenings
            .filter(
              (screening) =>
                (screening.venue_name || "Salle inconnue") === venueName,
            )
            .map((screening) => ({
              ...screening,
              visualRowStart:
                Math.floor((screening.startMinutes - dayStart) / 15) + 1,
              visualRowSpan: Math.max(
                1,
                Math.ceil((screening.endMinutes - screening.startMinutes) / 15),
              ),
            })),
        })),
      };
  ...
  function formatDayChipLabel(dayKey: string): string {
    ...
    const weekday = new Intl.DateTimeFormat("fr-CH", { weekday: "short" })
      ...
      .replace(".", "")
      .toLowerCase();
    const day = new Intl.DateTimeFormat("fr-CH", { day: "2-digit" }).format(
      date,
    );
    return `${weekday} ${day}`;
  }
  ...
  function findSelectedSibling(
    screening: PlanningScreening,
  ): PlanningScreening | undefined {
    return planningScreenings.value.find(
      (other) =>
        other.film_id === screening.film_id &&
        other.id !== screening.id &&
        (other.selection_status === "tentative" ||
          other.selection_status === "confirmed"),
    );
  }

  function findConflictingSelectedOtherFilm(
    screening: PlanningScreening,
  ): PlanningScreening | undefined {
    return planningScreenings.value.find(
      (other) =>
  ...
  function screeningStateClass(screening: PlanningScreening): string {
    ...
    if (screening.selection_status === "confirmed")
      return "planning__timeline-item--confirmed";
    if (screening.selection_status === "tentative")
      return "planning__timeline-item--tentative";
    if (screening.selection_status === "rejected")
      return "planning__timeline-item--rejected";
    if (screening.isAlternative || screening.derived_state === "disabled")
      return "planning__timeline-item--disabled";
    if (screening.derived_state === "conflict")
      return "planning__timeline-item--blocked";
  ...
  function screeningComparisonHints(screening: PlanningScreening): string[] {
    ...
    if (screening.isAlternative || screening.derived_state === "disabled") {
      ...
      if (selectedSibling?.starts_at) {
        hints.push(
          `Tu as déjà retenu ${formatDayChipLabel(selectedSibling.dayKey)} à ${selectedSibling.starts_at.slice(11, 16).replace(":", "h")}`,
        );
      }
    } else if (conflictingSelected?.starts_at) {
      hints.push(
        `Potentiel conflit avec ${conflictingSelected.film_title}, ${formatDayChipLabel(conflictingSelected.dayKey)} ${conflictingSelected.starts_at.slice(11, 16).replace(":", "h")}`,
      );
    }
  ...
  function screeningDecisionNote(screening: PlanningScreening): string {
    ...
    if (
      (screening.isAlternative || screening.derived_state === "disabled") &&
      selectedSibling?.starts_at
    ) {
  ...
  function selectedCountForDay(dayKey: string): number {
    ...
    return planningScreenings.value.filter(
      (screening) => screening.dayKey === dayKey && screening.isSelected,
    ).length;
  }

  function dayChipLabel(dayKey: string): string {
    return dayKey === FESTIVAL_VIEW_KEY
      ? "Festival entier"
      : formatDayChipLabel(dayKey);
  }

  async function setScreeningSelection(
    screeningId: number,
    nextStatus: Screening["selection_status"],
  ): Promise<void> {
  ...
  function focusScreening(
    screening: PlanningScreening | null | undefined,
  ): void {
    if (!screening) return;
